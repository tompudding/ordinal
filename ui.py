...
class UIElement(object):
    ...
    def CollidesAny(self, element, include_parent=True):
        ...
        if include_parent:
            if not (
                self.absolute.bottom_left.x > element.absolute.top_right.x
                or self.absolute.top_right.x < element.absolute.bottom_left.x
                or self.absolute.bottom_left.y > element.absolute.top_right.y
                or self.absolute.top_right.y < element.absolute.bottom_left.y
            ):
                return True
...
class TextBox(UIElement):
    ...
    def Position(self, pos, scale, colour=None, ignore_height=False):
        ...
        while i < len(self.quads):
            ...
            self.SetLetterVertices(i, absolute_bl, absolute_tr, self.level + self.extra_level + 0.6)
            if colour:
...
class TextBoxButton(TextBox, HoverableElement):
    def __init__(
        ...
        self.colour = colour
        self.extra_level = 0 if level is None else level
        super(TextBoxButton, self).__init__(
            parent,
            pos,
            tr,
            text,

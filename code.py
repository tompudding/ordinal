...
class Connector(ui.HoverableElement):
    ...
    def __init__(self, parent, bl, tr):
        ...
        if self.visible:
            ...
            self.circle_lines = [drawing.Line(globals.line_buffer) for i in range(self.circle_segments)]
            self.border = [drawing.Line(globals.line_buffer) for i in range(4)]
            self.arrow = [drawing.Line(globals.line_buffer) for i in range(3)]
    ...
    def UpdatePosition(self):
        ...
        for i in range(len(self.points)):
            self.circle_lines[i].SetVertices(
                self.GetAbsolute(self.points[i]),
                self.GetAbsolute(self.points[(i + 1) % len(self.points)]),
                self.level + 0.5,
            )
        ...
        self.arrow[0].SetVertices(
            self.GetAbsolute(Point(0, 0.5) + self.arrow_offset),
            self.GetAbsolute(Point(0.5, 0.5) + self.arrow_offset),
            self.level + 0.5,
        )
        self.arrow[1].SetVertices(
            self.GetAbsolute(Point(0.4, 0.4) + self.arrow_offset),
            self.GetAbsolute(Point(0.5, 0.5) + self.arrow_offset),
            self.level + 0.5,
        )
        self.arrow[2].SetVertices(
            self.GetAbsolute(Point(0.4, 0.6) + self.arrow_offset),
            self.GetAbsolute(Point(0.5, 0.5) + self.arrow_offset),
            self.level + 0.5,
        )
    ...
    def UpdateConnectedLineForward(self):
        if self.next and self.visible:
            ...
            self.connector_line.SetVertices(
                self.GetAbsolute(Point(0.5, 0.5)),
                self.next.GetAbsolute(Point(0.5, 0.5)),
...
class OutputButton(Connector):
    ...
    def Update(self, t):
        if self.connecting:
            self.connector_line.SetVertices(
                self.GetAbsolute(Point(0.5, 0.5)), self.root.mouse_pos, drawing.constants.DrawLevels.ui
            )
    ...
    def MouseButtonUp(self, pos, button):
        if self.connecting:
            ...
            if button == 1:
                ...
                if isinstance(hover, InputButton) and hover.prev is None:
                    self.next = hover
                    self.next.prev = self
                    self.connecting = False
    ...
    def MouseMotion(self, pos, rel, handled):
        if self.connecting:
            self.connector_line.SetVertices(
                self.GetAbsolute(Point(0.5, 0.5)), pos, drawing.constants.DrawLevels.ui
            )
            self.connector_line.SetColour(drawing.constants.colours.red)
...
class SourceOutputButton(OutputButton):
    ...
    def Hover(self):
        ...
        comingup = [next(self.parent.gen) for i in range(5)]
...
class SinkInputButton(InputButton):
    ...
    def Hover(self):
        ...
        self.root.SetHelpText(
            self.message + " ".join("%d" % v for v in self.parent.sequence[self.parent.matched :])
        )
...
class Number(ui.UIElement):
    ...
    def UpdatePosition(self):
        ...
        top_left = self.absolute.bottom_left + Point(
            -self.line_peturb, self.absolute.size.y + self.line_peturb
        )
        bottom_right = self.absolute.bottom_left + Point(
            self.absolute.size.x + self.line_peturb, -self.line_peturb
        )
...
class CodePrimitive(ui.UIElement):
    ...
    def __init__(self, parent, pos, colour):
        ...
        self.title_bar = ui.TitleBar(
            self, Point(0, 0.9), Point(1, 1), self.title, colour=self.colour, buffer=globals.colour_tiles
        )
        self.content = ui.Box(
            self,
            Point(0, 0),
            Point(1, 0.9),
            colour=drawing.constants.colours.dark_grey,
            buffer=globals.colour_tiles,
        )
        self.border = [drawing.Line(globals.line_buffer) for i in (0, 1, 2, 3)]
        if self.input:
...
class SourceSymbol(ui.UIElement):
    ...
    def UpdatePosition(self):
        ...
        self.lines[0].SetVertices(
            self.GetAbsolute(Point(0.5, 0.25)), self.GetAbsolute(Point(0.5, 0.75)), self.level + 0.5
        )
        self.lines[1].SetVertices(
            self.GetAbsolute(Point(0.4, 0.65)), self.GetAbsolute(Point(0.5, 0.75)), self.level + 0.5
        )
        self.lines[2].SetVertices(
            self.GetAbsolute(Point(0.6, 0.65)), self.GetAbsolute(Point(0.5, 0.75)), self.level + 0.5
        )
...
class SinkSymbol(ui.UIElement):
    ...
    def UpdatePosition(self):
        ...
        self.lines[1].SetVertices(
            self.GetAbsolute(Point(0.4, 0.35)), self.GetAbsolute(Point(0.5, 0.25)), self.level + 0.5
        )
        self.lines[2].SetVertices(
            self.GetAbsolute(Point(0.6, 0.35)), self.GetAbsolute(Point(0.5, 0.25)), self.level + 0.5
        )
        ...
        if number.num == self.sequence[self.matched]:
            ...
            if self.matched == len(self.sequence):
                ...
                globals.current_view.mode.Complete(
                    len(globals.current_view.blocks), globals.current_view.last_cycle
                )
                self.matched = 0
            if self.root.HelpShowing():
                self.root.SetHelpText(
                    self.inputs[0].message + " ".join("%d" % v for v in self.sequence[self.matched :])
                )
        else:
            # play bad noise
...
class CodeCreator(ui.HoverableElement):
    ...
    def Depress(self, pos):
        ...
        new_code = self.code_class(
            ...
            globals.current_view.GetRelative(create_pos),
